...
class Env(object):
    ...
    def run(self, is_training=False):
        ...
        # Store the initial state for first move player
        ...
        while not self.is_over():
            ...
            # Step with action for move player (player_id)
            next_state, next_player_id = self.step(action, self.agents[player_id].use_raw)
            # Save action. Add the action to the trajectory for move player
            trajectories[player_id].append(action)

            # Set the resulting state and next player
            state = next_state
